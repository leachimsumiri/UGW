<template>
	<div>
		...
		<b-table
			striped
			hover
			:items="filterEventsByUserDefinedRadius(userDefinedRadius)"
		...
		>
			...
			<template #cell(remove)>
				<button
					...
					@click="
						removeEvent({
							event: events.find((event) => event.id === 10),
						})
					"
				>
					X
...
<script>
	...
	export default {
		...
		computed: {
			...mapState(["locations", "events"]),
			...mapGetters(["filterEventsByUserDefinedRadius"]),
			computedFields() {
				return this.distanceCalculated
					? this.fields
					: this.fields.filter(
							(field) => field.key !== "current_description"
					  );
			},
		},
		methods: {
			...mapActions(["enrichEventsByCurrentDistance", "removeEvent"]),
			getUserLocation() {
				let options = {
				...
				const error = (err) => {
					console.log(
						"failed to retrieve user data with navigator.geolocation. Error:"
					);
					console.warn(`ERROR(${err.code}): ${err.message}`);
					console.log(err);
					console.log(
						"trying now to retrieve user geoinformation by ip address"
					);
			...
			getUserLocationByIpAddress() {
				$.getJSON(
					"https://api.ipinfodb.com/v3/ip-city/?format=json&key=" +
						IP_INFOS_KEY,
					(data) => {
						if (data.statusCode !== "OK") {
							console.warn(
								"error retrieving user location by ip address:"
							);
							console.log(JSON.stringify(data, null, 2));
						} else {
							console.log(
								"successfully retrieved user location by ip address:"
							);
							console.log(JSON.stringify(data, null, 2));
